...
final class FeaturedRepositoryMock: PFeaturedRepository {
    ...
    func setResults(movies: [Movie], shows: [Show])
    {
        moviesNowPlayingResult = Observable<[Movie]>.just(movies)
        showsOnTheAirResult = Observable<[Show]>.just(shows)
    }
}
